...
const props = defineProps({ modelValue: { type: Array, default: () => [] }, field: { type: Object, required: true }, feature: { type: String, required: true }, readOnly: Boolean, singleSelection: Boolean });
...
function updateSingle(fieldname, value) { emit("update:modelValue", value ? [{ [fieldname]: value }] : []); }
</script>
<template>
  <section class="ru-child"><header><div><h2>{{ field.label }}</h2><p v-if="field.description" class="ru-child__description">{{ field.description }}</p><small v-if="!singleSelection">{{ modelValue.length }} row{{ modelValue.length === 1 ? '' : 's' }}</small></div><button v-if="!readOnly && !singleSelection" class="ref-button ref-button--secondary" type="button" @click="add">+ Add {{ field.options === 'Has Role' ? 'role' : 'row' }}</button></header>
    <div v-if="singleSelection" class="ru-child__fields">
      <UniversalField
        v-for="child in field.child_fields.slice(0, 1)"
        :key="child.fieldname"
        :model-value="modelValue[0]?.[child.fieldname]"
        :field="{ ...child, label: field.label, required: field.required, read_only: readOnly || child.read_only }"
        :feature="feature"
        :parent-fieldname="field.fieldname"
        @update:model-value="updateSingle(child.fieldname, $event)"
      />
    </div>
    <template v-else>
