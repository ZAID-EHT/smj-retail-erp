...
async function load() { controller?.abort(); controller = new AbortController(); loading.value = true; error.value = null; expanded.value = {}; try { const result = await children(); roots.value = result.nodes; company.value = result.company || ""; } catch (caught) { if (caught.name !== "AbortError") error.value = caught; } finally { loading.value = false; } }
