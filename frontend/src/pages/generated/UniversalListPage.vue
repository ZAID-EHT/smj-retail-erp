...
const props = defineProps({ featureKey: { type: String, default: "" }, basePath: { type: String, default: "" } });
...
const modulePath = computed(() => props.basePath ? `/${props.basePath.split("/").filter(Boolean)[0] || "home"}` : `/${String(config.value?.feature?.module || "home").toLowerCase().replace(/\s+/g, "-")}`);
...
function open(name) { router.push(`${listPath.value}/${encodeURIComponent(name)}`); }
watch(() => [feature.value, props.basePath], () => { search.value = String(route.query.q || ""); config.value = null; Object.keys(appliedFilters).forEach((key) => delete appliedFilters[key]); Object.keys(draftFilters).forEach((key) => delete draftFilters[key]); load({ resetConfig: true }); }, { immediate: true });
...
<template><PageContainer><PermissionDenied v-if="error?.permissionDenied" /><ErrorState v-else-if="error" title="Unable to load records" :message="error.message" /><main v-else class="rug-page">
  <nav class="rug-breadcrumbs" aria-label="Breadcrumb"><RouterLink to="/home">Home</RouterLink><span>›</span><RouterLink :to="modulePath">{{ config?.feature?.module || 'Records' }}</RouterLink><span>›</span><strong>{{ title }}</strong></nav>
  <header :class="['rug-banner', `rug-banner--${accent}`]"><div><span class="rug-badge">Retail ERP page</span><h1>{{ title }}</h1><p>{{ config?.presentation?.description || `Browse and manage permitted ${title.toLowerCase()}.` }}</p></div><button v-if="config?.permissions?.can_create" class="rug-primary" type="button" @click="router.push(`${listPath}/new`)">+ New {{ config.feature.feature_label }}</button></header>
  <section class="rug-list-card">
    ...
    <div v-if="loading && !records.length" class="rug-skeleton" role="status"><i v-for="n in 8" :key="n" /></div><div v-else-if="!records.length" class="rug-empty"><SmjSearch size="22" decorative /><h2>No {{ title.toLowerCase() }} found</h2><p>Try changing the search or filters.</p></div>
